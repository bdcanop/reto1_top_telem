...
# Función para hacer ping a otros superpeers
def ping_superpeers():
    for superpeer in superpeers[:]:
        try:
            response = requests.get(f"http://{superpeer}/ping", timeout=5)
            if response.status_code != 200:
                raise Exception("Non-200 response")
        except Exception as e:
            print(f"Superpeer {superpeer} no responde. Eliminando de la lista.")
            superpeers.remove(superpeer)
...
def search():
    try:
        ...
        # Primero, hacer ping a los superpeers conocidos para actualizar la lista
        ping_superpeers()
